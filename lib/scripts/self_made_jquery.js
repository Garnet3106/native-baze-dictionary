...
class ExtendedNode {
    constructor(domNode) {
        let constructor = domNode.constructor;

        if(constructor === Array || constructor === ExtendedNodeList) {
            b
        }
...
class ExtendedNodeList {
    constructor(domNodeList) {
        ...
        // すべての子要素を初期化する
        if(domNodeList.length === undefined) {a}
    ...
    static joinNodeLists(array) {
        switch(array.length) {
            ...
            case 1: {
                let isExtendedNodeList = array[0].constructor === ExtendedNodeList;
                return isExtendedNodeList ? array[0] : new ExtendedNodeList(array[0]);
            }
            ...
            default: {
                ...
                for(let i = 0; i < array.length; i++) {
                    for(let j = 0; j < array[i].length; j++) {
                        let isExtendedNode = array[i][j].constructor === ExtendedNode;
                        let value = isExtendedNode ? array[i][j] : new ExtendedNode(array[i][j]);
                        result.push(value);
                    }
                }
    ...
    selected() {
        ...
        this.object.forEach(extNode => {
            ...
            console.log(extNode.object)

            if(isSelected)
                matched.push(extNode);
        });

        //console.log(matched)
        //console.log(ExtendedNodeList.joinNodeLists(matched))
        return ExtendedNodeList.joinNodeLists(matched);
    }
